...
def node_features(G, path = "../nets"):
  ...
  flpa = {}
  for c, comm in enumerate(nx.community.fast_label_propagation_communities(G, seed = 2026)):
    for i in comm:
      flpa[i] = c
  ...
  markov = {}
  for c, comm in enumerate(algorithms.markov_clustering(G).communities):
    for i in comm:
        markov[i] = c
        
  sbm = {}
  for c, comm in enumerate(SBM(G).communities):
    for i in comm:
      sbm[i] = c
      
  with open(path + "/" + G.name + ".nodes.features.tab", 'w') as file:
    file.write("mS#id\tmS#node\tC#degree\tC#pagerank\tC#closeness\tC#betweenness\tC#clustering\tD#flpa\tD#louvain\tD#markov\tD#sbm\tcD#class\n")
    for i, node in G.nodes(data = True):
      file.write("{:d}\t{:s}\t{:f}\t{:f}\t{:f}\t{:f}\t{:f}\t{:d}\t{:d}\t{:d}\t{:d}\t{:d}\n".format(i, node['label'], DC[i], PR[i], CC[i], BC[i], C[i], flpa[i], louvain[i], markov[i], sbm[i], node['_class']))
...
def edge_features(G, edges, path = "../nets"):
  ...
  with open(path + "/" + G.name + ".edges.features.tab", 'w') as file:
    file.write("mS#id1\tmS#id2\tmD#data\tC#preferential\tC#jaccard\tC#adamic_adar\tC#distance\tD#flpa\tD#louvain\tD#markov\tD#sbm\tcD#class\n");
    for data in edges:
      for c in edges[data]:
        for i, j in edges[data][c]:
          file.write(str(i) + "\t" + str(j) + "\t" + data + "\t" + str(preferential(G, i, j)) + "\t" + str(jaccard(G, i, j)) + "\t" + str(adamic_adar(G, i, j)) + "\t" + str(_dist(i, j)) + "\t" + ("1" if flpa[i] == flpa[j] else "0") + "\t" + ("1" if louvain[i] == louvain[j] else "0") + "\t" + ("1" if markov[i] == markov[j] else "0") + "\t" + ("1" if sbm[i] == sbm[j] else "0") + "\t" + str(c) + "\n")
