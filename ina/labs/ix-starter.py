...
algs = {"Known": lambda G: clustering(G), "Graph": lambda G: NodeClustering([range(len(G))], G, 'Graph'), "Isolates": lambda G: NodeClustering([[i] for i in range(len(G))], G, 'Isolates')} # "MCL": algorithms.markov_clustering, "EMMM": lambda G: algorithms.em(G, k = 2), "DCSBM": algorithms.sbm_dl
