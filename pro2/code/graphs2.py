...
def component(A, N, i):
  """
  Find connected component of node i in graph represented by adjacency list A considering only nodes in set N.
  Function returns list of node indices C in connected component of node i and removes them from set N.
  """
  C = []
  S = []
  N.remove(i)
...
def components(A):
  """
  Find connected components of graph represented by adjacency list A.
  Function returns list of lists C of node indices in connected components computed by `component(A, N, i)`.
  """
  C = []
  N = set(range(len(A)))
  while N:
...
def info(G):
  ...
  Method prints out graph name and type, number of (isolated) nodes and (self) edges, average node degree, size and number of connected components, and method running time.
  """
  start = time()
  (name, A, L) = G
