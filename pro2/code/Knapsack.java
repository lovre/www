...
public class Knapsack {
	...
	static void mercator(double volume, int stock) {
...
//			volumes.add(0.75);
		...
		Solution solution = knapsack(volume, prices, volumes);
		
		System.out.format("%12s | %.1f L\n", "Volume", volume);
		System.out.format("%12s | %.1f €\n", "Price", solution.value / 100.0);
		
		Map<String, Quantity> quantity = new HashMap<>();
		for (int i = 0; i < items.size(); i++)
			if (solution.items[i])
				if (!quantity.containsKey(items.get(i)))
					quantity.put(items.get(i), new Quantity(1, prices.get(i)));
				else
					quantity.get(items.get(i)).number++;
		
		for (String item: quantity.keySet())
			System.out.format("%10.0f € | %s (%dx)\n", quantity.get(item).number * quantity.get(item).price, item, quantity.get(item).number);

		System.out.println();
	}
	
	static Solution knapsack(double budget, List<Double> prices, List<Double> volumes) {
		int n = prices.size();
		...
		for (int i = 0; i < n; i++) {
			values[i] = (int)(100 * prices.get(i));
			weights[i] = (int)(100 * volumes.get(i));
		}
		
		return knapsack((int)(100 * budget), values, weights);
	}
...
class Quantity {
	...
	public Quantity(int number, double price) {
		this.number = number;
		this.price = price;
	}
