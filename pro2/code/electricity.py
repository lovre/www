...
regressors = {"kNN": neighbors.KNeighborsRegressor(), "LR": linear_model.LinearRegression(), "DT": tree.DecisionTreeRegressor(), "RF": ensemble.RandomForestRegressor(), "SVM": svm.SVR(), "MLP": neural_network.MLPRegressor(max_iter = 1500)}
...
## splits data into train instances (eleven months) and test instances (last month)

n = 30 * 24

train_instances = instances[:-n]
train_values = values[:-n]

test_instances = instances[-n:]
test_values = values[-n:]
