...
class EdgeException(GraphException):
  ...
  def __init__(self, i, j):
    super().__init__("edge between nodes with indices {:d} and {:d} does not exist".format(i, j))
...
class Graph:
  ...
  def component(G, N, i):
    """
    Find connected component of node i in graph G represented by class Graph considering only nodes in set N.
    Function returns a list of node indices C in connected component of node i and removes them from set N.
    """
    C = []
    S = []
    N.remove(i)
