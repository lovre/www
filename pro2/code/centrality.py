...
def tops(G, C, label = 'Centrality', n = 16):
  """
...
for name in ['got_kills', 'imdb_actors']:
  ...
  tops(G, lambda G: DC(G), 'Degree')
  tops(G, lambda G: EC(G), 'Eigenvector')
  tops(G, lambda G: PR(G), 'PageRank')
