...
class Graph:
  ...
  def add_edge(self, i, j):
    if not self.is_node(i):
      ...
      raise GraphException("loops on nodes are not supported")
    ...
    with open(os.path.join(path, name + '.net'), 'r') as file:
      for line in file:
        if re.match(r'^\*vertices', line):
          G = Graph(name, int(re.split(r'\s+', line)[1]))
  ...
  def write(G, path = '.'):
    ...
    with open(os.path.join(path, G.get_name() + '.net'), 'w') as file:
      ...
      for i in G.get_nodes():
        file.write('{:d}{:s}\n'.format(i + 1, ' "' + G.get_label(i) + '"' if G.get_label(i) is not None else ''))
...
if __name__ == "__main__":
  ...
  # constructs and prints out standard information of random graph R

  R = Graph("random", 100)
  
  while R.get_m() < 250:
    i = random.randint(0, R.get_n() - 1)
    j = random.randint(0, R.get_n() - 1)
    
    if i != j: # and not R.is_edge(i, j)
      R.add_edge(i, j)
