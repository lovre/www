...
import cartopy as map

RED, GRAY, LIGHT = [0.73, 0.33, 0.33], [0.66] * 3, [0.95] * 3
...
plt.plot([date for date in dates if cases[date] > 0], [cases[date] for date in dates if cases[date] > 0], zorder = 0, \
  marker = 'o', markersize = 3, linewidth = 0, c = GRAY, markeredgecolor = GRAY, label = "Cases (daily)")
plt.plot([date for date in dates if deaths[date] > 0], [deaths[date] for date in dates if deaths[date] > 0], zorder = 1, \
  marker = '*', markersize = 5, linewidth = 0, c = RED, markeredgecolor = RED, label = "Deaths (daily)")
...
countries = sorted(cases, key = lambda country: cases[country], reverse = True)
...
countries = sorted(cases, key = lambda country: cases[country] / details[country]["population"])

# maps number of COVID-19 cases and deaths by country

fig = plt.figure()

ax = plt.axes(projection = map.crs.PlateCarree())
ax.add_feature(map.feature.LAND, color = LIGHT)
ax.add_feature(map.feature.COASTLINE, linewidth = 0.75)
ax.add_feature(map.feature.BORDERS, linewidth = 0.75)
ax.set_extent([-25, 40, 30, 67.5])
ax.set_facecolor('white')

for i, country in enumerate(countries):
  plt.scatter([details[country]["longitude"]], [details[country]["latitude"]], zorder = 2, \
    marker = 'o', s = 750 * (cases[country] / details[country]["population"]), c = [GRAY + [0.75]], edgecolors = ['white'], \
    label = "Cases (per capita)" if country == "SVN" else None)
  plt.scatter([details[country]["longitude"]], [details[country]["latitude"]], zorder = 3, \
    marker = 'o', s = 25000 * (deaths[country] / details[country]["population"]), c = [RED + [0.75]], edgecolors = ['white'], \
    label = "Deaths (per capita)" if country == "SVN" else None)
  plt.text(details[country]["longitude"], details[country]["latitude"], details[country]["name"], zorder = 4, \
