...
with open("electricity.arff", 'r') as file:
  ...
  for line in file:
    if '?' not in line:
      values = [float(value) if i > 0 else datetime.strptime(value, '"%m/%d/%y %H:%M"') for i, value in enumerate(line.split(","))]
      data.append(values)
...
plt.xticks([datetimes[i * len(datetimes) // 4] for i in range(4)] + [datetimes[-1]])
...
# plots prediction for first half of test instances and stores image to PDF file
