...
fig.delaxes(ax1)
ax1 = fig.add_subplot(1, 2, 1, projection = map.crs.PlateCarree())
ax1.add_feature(map.feature.LAND, color = [0.95] * 3)
ax1.add_feature(map.feature.COASTLINE, linewidth = 0.75)

ax1.scatter(longitudes, latitudes, zorder = 2, marker = '*', s = 150, c = clusters, cmap = 'viridis', edgecolors = 'black')
  
ax1.set_extent([-12.5, 60, 10, 67.5])
ax1.set_title("Terrorist attacks")

# plots cluster centroids of terrorist attacks as bar charts

cmap = plt.get_cmap('viridis')
height = 1 / (len(centroids) + 1)

for i, centroid in enumerate(centroids):
  ax2.barh([(i + 1) * height + j for j in range(len(centroid))], centroid, height, color = cmap(i / (len(centroids) - 1)), edgecolor = 'black')
