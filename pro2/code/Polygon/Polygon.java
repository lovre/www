	...
	public static void main(String[] args) {
		...
		JPanel panel = new JPanel() {
			...
			public void paint(Graphics g) {
				...
				Point.Double point = new Point.Double(getWidth() / 2, getHeight() / 2 - r);
				for (int i = 0; i < n; i++) {
					alpha += 2 * Math.PI / n;
					
					Point.Double next = new Point.Double(getWidth() / 2 + r * Math.cos(alpha), getHeight() / 2 + r * Math.sin(alpha));
					g.drawLine((int)point.getX(), (int)point.getY(), (int)next.getX(), (int)next.getY());
